...
| 要素 | 仕様 |
|---|---|
| 警告表示 | サーバの `warnings[]` を保持し、クライアント警告と合わせて画面上部の黄色帯に表示。ただしサーバの「品目合計 …」不一致警告（4.3）は**別枠では表示しない**（先頭が「品目合計」の警告文を保持時に除外する）。クライアントが同じ不一致をライブ再計算し、サーバと同じ税抜ヒント（`TAX_RATES = 1.10 / 1.08`、±1円以内で一致すれば「（品目が税抜表示の可能性: ×1.10 で一致）」を付加）付きで表示するため、二重表示を避ける。日付補完など他のサーバ警告はそのまま表示する。カテゴリごとに `price < 0` の品目行があり、同じカテゴリに `price > 0` の品目行がない場合、該当する負数行ごとに「値引き行（<name> <price>円）が <カテゴリ表示名> にありますが、<カテゴリ表示名> の品目がありません。値引きは対象品目と同じカテゴリにしてください。」を表示（child=子ども費、couple=夫婦生活費、excluded=対象外）。品目合計とレシート合計の不一致警告と同様に、入力・カテゴリ変更・行追加削除・1行にまとめる・内訳復元のたびに再計算。保存はブロックしない（2026-09-13 決定） |
